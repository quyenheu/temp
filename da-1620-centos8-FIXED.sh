...
# [FIX #1] Create localhost.zone locally instead of downloading - avoid 404 error
if [ ! -f /etc/named/localhost.zone ]; then
	mkdir -p /etc/named
	cat > /etc/named/localhost.zone << 'ZONEOF'
$TTL 86400
...
ZONEOF
	chmod 644 /etc/named/localhost.zone
	chown named:named /etc/named/localhost.zone
fi
...
# [FIX #2] Skip LEGO build to avoid 404 error on lego files
# The old lego files no longer exist on the server - skip this build
# If you need LEGO, you can enable it later via custombuild
if [ -f "$CB_OPTIONS" ]; then
	sed -i 's/^lego=.*/lego=no/' $CB_OPTIONS 2>/dev/null
	sed -i 's/^letsencrypt=.*/letsencrypt=no/' $CB_OPTIONS 2>/dev/null
fi

# Original lines (commented out due to 404 errors):
# $BUILD lego
# $BUILD letsencrypt
