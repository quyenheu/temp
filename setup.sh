...
###############################################################################
# setup.sh - DIRECTADMIN FIXED 404 ERROR
# Fixed: Thêm fallback khi download files.sh fail
###############################################################################
...
###############################################################################
# FIX 404 ERROR - Download files.sh with fallback
###############################################################################
...
echo "Downloading files.sh from: $SERVER/$FILES_PATH/files.sh"
wget ${WGET_OPTION} -O $FILES $SERVER/$FILES_PATH/files.sh 2>/dev/null

# FIX: If download fails, create fallback files.sh
if [ ! -s $FILES ]; then
	echo "⚠ Warning: Unable to download files.sh (404 Not Found)"
	echo "Creating fallback files.sh..."
	
	cat > $FILES << 'EOFFILES'
#!/bin/bash
# Fallback files.sh - Auto-generated when download fails
...
gd="gd"
krb5_libs="krb5-libs"
krb5_devel="krb5-devel"
e2fsprogs="e2fsprogs"
e2fsprogs_devel="e2fsprogs-devel"
perl_dbi="perl-DBI"
patch="patch"
quota="quota"
export bind_utils bind bison flex gd krb5_libs krb5_devel e2fsprogs e2fsprogs_devel perl_dbi patch quota
EOFFILES
	
	echo "✓ Fallback files.sh created"
fi
