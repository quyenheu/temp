...
# ============================================================================
# [CORRECT FIX] Disable LEGO & LETSENCRYPT to avoid 404 errors
# This MUST be done BEFORE any build attempts
# ============================================================================
echo "Disabling LEGO and LETSENCRYPT builds to prevent 404 errors..."

if [ -f "$CB_OPTIONS" ]; then
	# Disable LEGO (old build causing 404)
	sed -i 's/^lego=.*/lego=no/' $CB_OPTIONS
	sed -i 's/^letsencrypt=.*/letsencrypt=no/' $CB_OPTIONS
	
	echo "LEGO status:"
	grep "^lego=" $CB_OPTIONS
	echo "LETSENCRYPT status:"
	grep "^letsencrypt=" $CB_OPTIONS
else
	echo "Warning: options.conf not found yet, will disable after custombuild creates it"
fi

# Original lines that cause 404 errors - COMMENTED OUT:
# $BUILD lego
# $BUILD letsencrypt
